#!/usr/bin/env bash
# Customised version of Sexy bash prompt by twolfson
...
#########
# customization (symbol/color/PS1 format)
...
PS1="\[$C_BLUE\]\u\[$C_RESET\]\
\[$C_BLUE\]@\[$C_RESET\]\
\[$C_BLUE\]\h\[$C_RESET\]\
\[$C_WHITE\]:\[$C_RESET\]\
\[$C_RED\]\w\[$C_RESET\]\
\$( sexy_bash_prompt_is_on_git && \
  echo -n \" \[$C_WHITE\]on\[$C_RESET\] \" && \
  echo -n \"\[$C_GREEN\]\$(sexy_bash_prompt_get_git_info)\" && \
  echo -n \"\[$C_GREEN\]\$(sexy_bash_prompt_get_git_progress)\" && \
  echo -n \"\[$C_WHITE\]\")\[$C_RESET\] \
\[$C_WHITE\]$(sexy_bash_prompt_get_prompt_symbol) \[$C_RESET\]"
